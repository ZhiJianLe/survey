...
public class SurveyController {
    ...
    public Map<String,Object> create(@RequestBody Survey survey, HttpServletRequest request){
        System.out.println("-----------------------------------------");
        Admin currentAdmin= SessionUtils.getAdmin(request);
        survey.setCreator(currentAdmin.getId());
        survey.setState(Survey.state_create);
        survey.setAnon(survey.getAnon()!=null?0:1);
        int code=surveyService.create(survey);
        //失败的情况下
        if(code<=0) {
    ...
    public Map<String,Object> delete(String ids){
        int code=surveyService.deleteBatch(ids);
        //失败的情况下
        if(code<=0) {
            return MapControl.getInstance().error().getMap();
        }
    ...
    public Map<String,Object> update(@RequestBody Survey survey){
        survey.setAnon(survey.getAnon()!=null?0:1);
        int code=surveyService.update(survey);
        //失败的情况下
        if(code<=0) {
            return MapControl.getInstance().error().getMap();
        }
